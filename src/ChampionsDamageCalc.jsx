...
// 発動チェックで計算上の天気をセット。あめふらし等は「天気を作る」、サンパワー/すなのちから/すなはきは「対応天気にして効果発動」
const WEATHER_ABILITY = { あめふらし: "あめ", ひでり: "はれ", すなおこし: "すなあらし", ゆきふらし: "ゆき", サンパワー: "はれ", すなのちから: "すなあらし", すなはき: "すなあらし" };
...
const DAMAGE_ABILITIES = new Set([
  ...
  ...Object.keys(ABILITY_IMMUNE), ...Object.keys(ABILITY_TYPE_MUL), ...Object.keys(WEATHER_ABILITY),
...
export default function ChampionsDamageCalc() {
  ...
  const setEnemyIdx = (idx) => { enemyCfgRef.current = null; (atkOnRight ? setAtkIdx : setDefIdx)(idx); }; // 認識/手動指定の相手は構成情報を持たない
  ...
  useEffect(() => {
    ...
    atkSnapRef.current = {}; defSnapRef.current = {}; // ポケ別スナップに残る旧ランクも破棄（交代で蘇らないように）
  ...
  useEffect(() => setAtkAutoOff(false), [atkAbility]);
  useEffect(() => setDefAutoOff(false), [defAbility]);

  // 自動判定特性は常に計算へ反映（条件は効果処理側で判定）、手動特性はチェックオン時のみ
  const atkAbilityEff = isAutoAbility(atkAbility) ? (atkAutoOff ? "なし" : atkAbility) : (atkAbilityOn || (atkAbility === "こんじょう" && burn)) ? atkAbility : "なし"; // 自動特性は手動オフ可。こんじょうはやけど中なら自動オン
  ...
  const atkTypes = (atkAbilityEff === "てんきや" && FORECAST_TYPE[weather]) ? [FORECAST_TYPE[weather]] : attacker.types;
  ...
  const atkIE = itemEffect(atkItem, "atk");
  const defIE = itemEffect(defItem, "def");
  const effPowerNoItem = effPower; // 威力アイテム補正の前（攻撃力推定で持ち物別に逆算するため）
  ...
  let stabMul = 1.5, aAbilityMul = 1, ignoreBurn = false, protean = false, atkAbNote = "";
  {
    ...
    if (ab === "スキルリンク" && move.hits) atkAbNote = `スキルリンク: 連続技${move.hits[1]}回固定`;
  ...
  const defRankEff = atkAbilityEff === "てんねん" ? 0 : defRank;
  ...
  let condNote = "";
  {
    ...
    if (moveKey === "ライジングボルト" && terrain === "エレキ" && !defTypes.includes("ひこう")) { effPower = f(effPower * 2); condNote = "エレキF 威力×2"; }
    if (moveKey === "ワイドフォース" && terrain === "サイコ" && !atkTypes.includes("ひこう")) { effPower = f(effPower * 1.5); condNote = "サイコF 威力×1.5"; }
  ...
  const result = useMemo(() => {
    ...
    if (parentalBond) {
      ...
      const lo = move.hits[0], hi = move.hits[1];
    ...
    const hitLabel = !isMulti ? "1" : nHits === 0 ? `${move.hits[0]}〜${move.hits[1]}` : `${nHits}`;
    return {
  ...
  }, [attacker, defender, move, effType, effPower, isFixedMove, fixedDmgVal, isPhysical, isMulti, hits, atkSp, atkNature, atkRank, crit, burn, helpingHand, hpSp, bSp, dSp, bNature, dNature, defRank, wall, weather, critEff, lightBall, atkItem, defItem, atkAbilityEff, defAbilityEff, stabMul, aAbilityMul, ignoreBurn, protean, noWeather]);
  ...
  const atkInference = useMemo(() => {
    ...
    return { sets };
  }, [dmgTaken, attacker, defender, move, effType, effPower, effPowerNoItem, atkItem, atkInferItem, exclBandGlasses, isPhysical, isMulti, atkRank, crit, burn, helpingHand, bSp, dSp, bNature, dNature, defRank, wall, weather, critEff, lightBall, defItem, stabMul, aAbilityMul, ignoreBurn, protean, noWeather, defAbilityEff, atkAbilityEff]);
  ...
  return (
    <div className="root" style={{ "--accent": accent, background: rootBg }}>
      ...
      <style>{`
        ...
        .rank-ab-btn{display:flex;align-items:center;gap:8px;background:#0e1320;border:1px solid var(--brand-border);border-radius:7px;color:#d6deec;font-size:12px;padding:6px 10px;cursor:pointer;text-align:left}
        ...
        .dock-log-item{display:flex;align-items:center;gap:8px;font-size:12px;color:#d6deec;background:#0e1320;border:1px solid #232d44;border-radius:6px;padding:4px 9px;cursor:pointer}
        ...
        <div className="grid3">
              ...
              {isMulti && (
                <div className="field">
                  ...
                  {atkAbilityEff === "スキルリンク" ? (
                    <select className="rank" value={move.hits[1]} disabled>
                      <option value={move.hits[1]}>{move.hits[1]}発(スキルリンク確定)</option>
                    </select>
                  ) : (
                    <select className="rank" value={move.hits[0] === move.hits[1] && hits === 0 ? move.hits[0] : hits} onChange={(e) => setHits(Number(e.target.value))}>
                      {move.hits[0] !== move.hits[1] && <option value={0}>{move.hits[0]}〜{move.hits[1]}(確率)</option>}
                      {[1, 2, 3, 4, 5].map((n) => <option key={n} value={n}>{n}発</option>)}
                    </select>
                  )}
                </div>
              )}
          ...
          <div className="center-col">
            ...
            <label className="center-field"><span className="field-label">天気</span>
              <select className="rank" value={weatherSel} onChange={(e) => setWeather(e.target.value)}>
            ...
            {resultExpanded && (
              <div className="dock-details">
                <div className="meta-tags">
                  ...
                  {isMulti && <span className="tag">連続 {result.hitLabel}回{atkAbilityEff === "スキルリンク" ? "（スキルリンク確定）" : result.nHits === 0 ? "（確率 35/35/15/15%）" : ""}</span>}
