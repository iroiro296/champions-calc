...
const abilityOptions = (poke) => {
  ...
  return (poke.abilities?.length ? poke.abilities : ["なし"]).map((x) => ({ x, u: u[x] })).sort((a, b) => (b.u ?? -1) - (a.u ?? -1));
};
// 採用率データの基準時刻を日本時間で整形
...
export default function ChampionsDamageCalc() {
  ...
  useEffect(() => { setAtkAutoOff(false); setProteanType(""); }, [atkAbility]); // 特性を変えたらへんげんじざいの変化後タイプもリセット
        ...
        <div className="grid3">
          ...
          <div className="center-col">
            ...
            <label className="center-field"><span className="field-label">フィールド</span>
              <select className="rank" value={terrainEff} onChange={(e) => setTerrain(e.target.value)}>
